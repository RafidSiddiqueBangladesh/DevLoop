  ...
  return (
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="flex-1 container mx-auto px-4 py-12 max-w-7xl">
        {/* Welcome Section */}
        <div className="mb-12">
          <h1 className="text-4xl font-bold mb-2">{t('dashboard.welcome')}, User 👋</h1>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          <div className="rounded-xl bg-card border border-border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">{t('dashboard.inventory')}</p>
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">{t('dashboard.expiringItems')}</p>
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">{t('dashboard.weeklySpending')}</p>
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm text-muted-foreground mb-1">{t('dashboard.budget')}</p>
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-12">
          <div className="rounded-xl bg-card border border-border p-6">
            <h2 className="text-xl font-bold mb-6">Weekly Consumption Pattern</h2>
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={consumptionData}>
                ...
                <Tooltip />
                <Line type="monotone" dataKey="vegetables" stroke="hsl(var(--primary))" />
                <Line type="monotone" dataKey="fruits" stroke="hsl(var(--secondary))" />
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={budgetData}>
                ...
                <YAxis stroke="var(--muted-foreground)" />
                <Tooltip />
                <Bar dataKey="budget" fill="hsl(var(--primary))" opacity={0.5} />
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          <div className="rounded-xl bg-card border border-border p-6">
            <h2 className="text-xl font-bold mb-4">{t('dashboard.recentLogs')}</h2>
            <div className="space-y-3">
              {[
                ...
                <div key={idx} className="flex justify-between items-center p-3 rounded-lg hover:bg-accent transition-colors">
                  <div>
                    <p className="font-medium">{log.item}</p>
                    <p className="text-sm text-muted-foreground">{log.quantity}</p>
                  </div>
          ...
          <div className="rounded-xl bg-card border border-border p-6">
            <h2 className="text-xl font-bold mb-4">{t('dashboard.recommendations')}</h2>
            <div className="space-y-3">
              {[
                { title: 'Use expiring tomatoes', reason: 'Expires in 2 days' },
                { title: 'Budget-friendly recipe idea', reason: 'Based on your items' },
                { title: 'Composting opportunity', reason: 'You have vegetable scraps' },
                { title: 'Seasonal vegetables available', reason: 'Fresh mango season' },
                { title: 'Storage tips for dairy', reason: 'Keep milk refrigerated' },
              ].map((rec, idx) => (
                <div key={idx} className="p-3 rounded-lg bg-gradient-to-r from-primary/5 to-secondary/5 border border-primary/10">
